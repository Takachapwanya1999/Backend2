...
export const Header = () => {
      ...
      <div className="max-w-screen-xl mx-auto px-4 sm:px-6">
        ...
        <div className="flex items-center justify-between py-3">
          ...
          <Link to="/" className="flex items-center group">
            <img src="/images/main-logo.png" alt="Airbnb" className="w-10 h-10 mr-2 transition-transform group-hover:scale-105" />
            <span className="text-2xl font-extrabold tracking-tight text-rose-500 hidden sm:inline" style={{letterSpacing: '-0.02em'}}>airbnb</span>
          </Link>
  ...
  <div className="hidden lg:flex items-center justify-center gap-10 text-gray-700 dark:text-slate-200">
    <button
      ...
      className={`flex items-center gap-2 px-3 py-2 relative font-semibold transition-colors ${activeTab === 'homes' ? 'text-gray-900' : 'hover:text-gray-900'}`}
      style={{fontSize: '1rem'}}
    >
      <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M10 20v-6h4v6h5v-8h3L12 3 2 12h3v8z"/></svg>
      <span>Homes</span>
      {activeTab === 'homes' && <span className="absolute -bottom-1 left-1/2 -translate-x-1/2 h-1 w-8 bg-rose-500 rounded-full" />}
    </button>
    <button
      onClick={() => handleTabClick('experiences')}
      className={`flex items-center gap-2 px-3 py-2 relative font-semibold transition-colors ${activeTab === 'experiences' ? 'text-gray-900' : 'hover:text-gray-900'}`}
      style={{fontSize: '1rem'}}
    >
      ...
      <span className="bg-rose-500 text-white text-[10px] px-2 py-0.5 rounded-full font-bold ml-1">NEW</span>
      {activeTab === 'experiences' && <span className="absolute -bottom-1 left-1/2 -translate-x-1/2 h-1 w-8 bg-rose-500 rounded-full" />}
    </button>
    <button
      onClick={() => handleTabClick('services')}
      className={`flex items-center gap-2 px-3 py-2 relative font-semibold transition-colors ${activeTab === 'services' ? 'text-gray-900' : 'hover:text-gray-900'}`}
      style={{fontSize: '1rem'}}
    >
      ...
      <span>Services</span>
      <span className="bg-gray-700 text-white text-[10px] px-2 py-0.5 rounded-full font-bold ml-1">NEW</span>
      {activeTab === 'services' && <span className="absolute -bottom-1 left-1/2 -translate-x-1/2 h-1 w-8 bg-rose-500 rounded-full" />}
    </button>
  </div>
